// Agent 共绘 · 内建本地中继轮询客户端
//
// 职责：当用户在 KaiBoard 的「✨ AI → AI 面板」里打开「Agent 共绘」后，页面自己长轮询本地中继
 ...
 * 用途：长轮询时上报给中继，让中继能把带 boardId 的命令路由到正确的页面。
 * 缺失（老调用方未注入）时不上报，中继退回原有"先到先得"派发，行为不变。
 */
let getBoardIdRef: (() => string | undefined) | null = null;
...
async function pollLoop() {
  ...
  while (running) {
    ...
    try {
      ...
      const bid = getBoardIdRef?.() || "";
      const boardQs = bid ? `&board=${encodeURIComponent(bid)}` : "";
      const res = await fetch(`${url}/cmd?token=${encodeURIComponent(bridgeToken)}${boardQs}`, {
        method: "GET",
