...
> **一句话定位**：无需注册、不上传云端，数据只留在你自己的设备上；为个人单人使用设计，不做实时协同。在绘图内核之上补齐了"多文件 / 多画板管理"这一能力（Excalidraw 原生缺失），并提供回收站与跨设备平行迁移。**可选**外接 AI Agent 共绘 —— **默认关闭**，用不用在你。
...
- **Agent 共绘（可选）**：开启后 AI Agent 可直接在你**当前打开的画板**上作图 —— 实时增删元素、可撤销、落板即见，无需导出 / 导入的来回。**默认关闭**；只与本机 `127.0.0.1` 中继通信，画布数据不出本机。见 [`docs/AGENT.md`](docs/AGENT.md)
...
- 第三方依赖（含作为绘图内核的 Excalidraw）的许可证见 [THIRD_PARTY_LICENSES.md](./THIRD_PARTY_LICENSES.md)。
